...
public sealed partial class TeacherService(
    ...
    ILogger<TeacherService> logger) : ITeacherService
{
    ...
    public async Task<PagedResponse<TeacherListItemResponse>> ListAsync(
    ...
    public async Task<TeacherDetailResponse> CreateAsync(
    ...
    public async Task<TeacherDetailResponse> UpdateAsync(
        Guid id,
    ...
    public async Task<TeacherDetailResponse> UpdateAttendancePolicyAsync(
        Guid id,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new AppValidationException(
                "Cửa sổ sửa điểm danh không hợp lệ.",
                new Dictionary<string, string[]>
                {
                    ["attendanceEditWindowDays"] = ["Giá trị phải từ 1 đến 7 ngày."]
                },
                ProblemCodes.InvalidAttendanceEditWindow);
        }
        ...
        var oldVersion = teacher.Version;
        teacher.AttendanceEditWindowDays = checked((short)request.AttendanceEditWindowDays);
        teacher.Version++;
        teacher.UpdatedAt = timeProvider.GetUtcNow();
        AddAudit(actor, "Teacher.AttendancePolicyUpdated", teacher,
            new { attendanceEditWindowDays = oldValue, version = oldVersion },
            new { teacher.AttendanceEditWindowDays, teacher.Version });
        await SaveTeacherChangesAsync(cancellationToken, teacher.Id);
        await transaction.CommitAsync(cancellationToken);
    ...
    private static bool Matches(
        TeacherListItemResponse candidate,
        string foldedSearch,
        string searchDigits) =>
    ...
    private static PagedResponse<TeacherListItemResponse> CreatePage(
    ...
    private static IQueryable<TeacherListItemResponse> ProjectList(IQueryable<Teacher> query) =>
        query.Select(x => new TeacherListItemResponse(
            x.Id,
            x.UserId,
            x.TeacherCode,
            x.User.FullName,
    ...
    private static IQueryable<TeacherDetailResponse> ProjectDetail(IQueryable<Teacher> query) =>
        query.Select(x => new TeacherDetailResponse(
            x.Id,
            x.UserId,
            x.TeacherCode,
            x.User.FullName,
            ...
            x.ResponsibleGroups
                ...
                .Select(group => new TeacherGroupSummaryResponse(
                    group.Id,
                    group.Code,
                    group.Name,
                    group.Status,
