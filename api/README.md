...
GET    /api/v1/students/{id}/assessment-results
PATCH  /api/v1/students/{id}/assessment-results
...
POST   /api/v1/assessment-sheets/{id}/submit-results/preview
POST   /api/v1/assessment-sheets/pdf-archive
...
- Cập nhật kết quả trực tiếp dùng `GET` và `PATCH /students/{id}/assessment-results`, chỉ dành cho `Admin`/`SuperAdmin`. `GET` chỉ đọc toàn bộ danh mục `Assessment` và mirror `AssessmentSheetLatest`/`AssessmentRecordLatest` trong PostgreSQL, không gọi hoặc fallback sang Google; response gồm `student: { id, studentCode, fullName, nickName, dateOfBirth, status }` và `items: [{ assessmentId, code, name, groupLv1Name, groupLv2Name, groupLv3Name, rowIndex, grade, note, version }]`. `version` là token opaque theo trạng thái catalog/latest của từng mục.
- `PATCH /students/{id}/assessment-results` nhận `{ items: [{ assessmentId, expectedVersion, grade, note }] }`; `grade` là `A|B|C|D|null`, note tối đa 2.000 ký tự (trim, rỗng thành null). Backend kiểm tra toàn bộ DB version trước khi gọi Google; stale trả `409 AssessmentResultsVersionConflict` cùng `conflicts: [{ assessmentId, currentVersion, currentGrade, currentNote }]`. Sau đó API đối chiếu Google với baseline DB; drift trả `409 AssessmentResultsSourceOutOfSync` cùng `sourceConflicts: [{ assessmentId, databaseGrade, databaseNote, sourceGrade, sourceNote }]`. Khi hợp lệ, API batch-update các ô thực sự đổi, readback, rồi cập nhật từng latest record bị sửa; note-only được giữ và grade/note cùng rỗng sẽ xóa latest record. Nếu Google đã ghi nhưng readback/mirror lỗi, trả `AssessmentResultsPostWriteFailed` với `googleWriteSucceeded: true`; client không được tự retry.
...
- `POST /assessment-sheets` nhận `records: [{ assessmentId, latestGrade, note }]`; `latestGrade`/`note` là dữ liệu gợi ý gần nhất do UI gửi theo mục đã chọn và được lưu snapshot vào `planGrade`/`planNote` khi tạo record. `Final*` luôn để trống khi tạo mới. `records` **được phép rỗng** (`[]`) — tạo bảng đánh giá không có mục nào; người dùng thêm mục sau ở màn chỉnh sửa. Khi có mục thì vẫn phải hợp lệ và không trùng. Các ràng buộc khác (học sinh tồn tại + active) giữ nguyên.
...
- `status` nhận `Open` | `Planed` | `Done` | `Canceled` (lưu dạng chuỗi, không migration schema). `Canceled` chỉ là **nhãn phân loại** "đánh giá không làm nữa": không side-effect nghiệp vụ, vẫn cho sửa metadata/records như `Open`, không đặt `doneDate`.
- `PUT /assessment-sheets/{id}` và `PUT /assessment-sheets/{id}/records` bị chặn (`409 AssessmentSheetDone`) khi `status = Done`; các trạng thái khác (`Open`/`Planed`/`Canceled`) đều sửa được. `PUT /assessment-sheets/{id}` sửa metadata (giáo viên, ghi chú, ngày, feedback) khi `status ∈ {Open, Canceled}`, còn khi `Planed`/`Done` chỉ nhận link PDF. `PUT /assessment-sheets/{id}/status` luôn chuyển được **giữa mọi trạng thái** theo cả hai chiều, không cần lý do (không có transition rule).
- `DELETE /assessment-sheets/{id}` xóa vĩnh viễn bảng đánh giá và toàn bộ `AssessmentRecord` phụ thuộc trong database của app, áp dụng cho mọi trạng thái và trả `204 No Content`. Endpoint không gọi Google, không xóa file PDF/Google Drive, không xóa danh mục `Assessment` hay dữ liệu mirror `AssessmentSheetLatest`/`AssessmentRecordLatest`; id không tồn tại trả `404 AssessmentSheetNotFound`. Quyền giữ cùng convention AssessmentSheet: `Teacher`/`Admin`/`SuperAdmin`.
...
- Sửa tên nhóm `groupLv2Name`/`groupLv3Name` trên ô merge của `records-panel` (ASH-GRP-01) chỉ đổi state UI; khi bấm "Lưu thay đổi" nó đi kèm `PUT /assessment-sheets/{id}/records`. `AssessmentSheetRecordRequest` có thêm `groupLv2Name`/`groupLv3Name` (nullable, tối đa 500). Backend ưu tiên: giá trị request (đã trim, bỏ trống coi như không gửi) → snapshot tùy chỉnh trước đó (map theo `Assessment.Code`) → tên nhóm của `Assessment` gốc. Vì vậy tên nhóm do người dùng sửa không mất khi sắp STT, thêm/xóa mục hay lưu ghi chú, và cũng không cần endpoint riêng.
- `PATCH /assessments/group` cập nhật `groupLv2Name` (`level=2`) hoặc `groupLv3Name` (`level=3`) trên bảng `Assessment` danh mục cho tập `assessmentCodes` được gửi lên, đặt tất cả về `name` mới. Chỉ `Admin|SuperAdmin` (Teacher trả `403`); code không tồn tại đủ trả `404 AssessmentNotFound`, code trùng trả `409 SnapshotChanged`. Trả `{ updatedCount }` và ghi audit `Assessment.GroupUpdated` (kèm `AssessmentCodes`, không lưu secret). Endpoint này không đụng snapshot của bất kỳ sheet nào; UI thường gọi kèm việc sửa snapshot tại chỗ để hiển thị nhất quán. Tên gốc vẫn có thể bị lần `sync-assessments` sau ghi đè.
...
- `GET /assessment-sheets` (ASH-FB-W4) nhận thêm `responsibleTeacherId` (Guid) để lọc theo giáo viên phụ trách; các filter cũ (`search`, `studentId`, `status`, `dateFrom`/`dateTo`) và paging/sort server-side giữ nguyên. `search` cũng khớp theo tên gọi ở nhà của học sinh.
- `AssessmentSheetListItemResponse` có `studentNickName` và `studentDateOfBirth` lấy từ `StudentSnapshot` (không migration) — UI danh sách hiển thị mã, tên gọi và tuổi theo năm/tháng ở dòng phụ của cột học sinh.
- `POST /assessment-sheets/pdf-archive` (ASH-FB-W4, Bulk Action màn danh sách): body `{ ids: Guid[] (1..500), kind: "Plan"|"Result", format: "Pdf"|"Images" }`. Một bản ghi `format=Pdf` trả trực tiếp `application/pdf`; một bản ghi `format=Images` trả trực tiếp `image/png` nếu PDF chỉ có một trang. Nhiều bản ghi luôn trả `application/zip`; `Images` cũng trả ZIP khi PDF có từ hai trang trở lên. Backend tải toàn bộ PDF từ Google Drive (link Drive không fetch trực tiếp từ browser được). Tên file trong zip = **tên gốc trên Google Drive** (đụng tên thêm ` (2)`), **nằm phẳng ở gốc zip** — không tạo thư mục riêng từng bảng. `format=Images` render từng trang PDF thành PNG bằng PDFium/SkiaSharp (`PDFtoImage`), tên `<tên gốc không đuôi> - trang 001.png`. Bảng không có link / tải lỗi / render lỗi bị **bỏ qua** và ghi vào `_bo-qua.txt` trong zip (response vẫn `200`). Quyền: `Teacher`/`Admin`/`SuperAdmin`.
...
- Cơ chế lưu Drive (cập nhật 2026-09-09): nếu `PlanFileLinkPdf`/`ResultFileLinkPdf` hiện tại chứa Drive file id, backend dùng `Files.Update` để thay nội dung PDF ngay trên file đó và giữ ổn định id/link. Chỉ khi chưa trích được existing file id mới dùng `Files.Create` trong `Student.DriveFolderId`.
- `submit-results` ghi nhãn `finalGrade` và `finalNote` vào `[F0.ĐG]` (dò `E16:E`/`H16:16`, `finalNote` nằm ở cột kế bên cột kết quả của học sinh), chỉ ghi cell có thay đổi, audit từng cell và set `submissionDate`. Sau mỗi submit, API cập nhật ngay từng latest record tương ứng trong DB, kể cả khi Google không có ô thay đổi, khi chỉ có note hoặc khi xóa cả grade/note; các latest record ngoài bảng submit không bị thay đổi. Lỗi Google/Drive thật trả `409 AssessmentSheetGoogleOperationFailed`.
- `submit-results/preview` là dry-run của `submit-results`: đọc và đối chiếu `[F0.ĐG]` rồi trả `{ gradeSummary, totalRecords, totalChangedCells, changes }` mà **không ghi**. `gradeSummary` là mảng cố định 5 mục theo thứ tự `Đạt +`, `Hỗ trợ +`, `Hỗ trợ -`, `Chưa đạt -`, `Chưa có kết quả` (`grade` = `null` cho mục cuối), mỗi mục `{ grade, label, count }` đếm trên toàn bộ record của bảng. `changes[]` là các ô sẽ đổi: `{ cell (vd `H45`), kind (`FinalGrade`|`FinalNote`), assessmentCode, assessmentName, currentValue, newValue }`. UI dùng để hiện popup xác nhận; nếu `totalChangedCells = 0` thì không cần gọi `submit-results`.
...
- `POST /google-sheets/sync-assessments` giữ **nguyên xuống dòng** cho `Assessment.Name`/`GroupLv1Name`/`GroupLv2Name`/`GroupLv3Name`: `AssessmentSyncTextNormalizer` chỉ `Trim()` hai đầu, không gộp CRLF/space thừa/dòng trống bên trong. Newline được giữ suốt từ sync → `AssessmentRecord.AssessmentSnapshot` (kể cả qua `replaceRecordSnapshots`, `PUT .../records`, import Excel). Tên/nhóm rỗng hoàn toàn vẫn thành `null`.
- `POST /google-sheets/sync-assessments` nhận thêm `replaceRecordSnapshots` (không bắt buộc; bỏ trống = hành vi mặc định trên). Khi có, sau bước rebuild catalog, API ghi đè các trường **đã chọn** từ `Assessment` mới vào snapshot đông cứng `AssessmentRecord.AssessmentSnapshot` — khớp theo `AssessmentSnapshot.Code == Assessment.Code`, chỉ với `AssessmentSheet` có `AssessmentSheetStatus` nằm trong `sheetStatuses`. Cờ trường: `name`, `groupLv1Name`, `groupLv2Name`, `groupLv3Name`, `rowIndex` (không có `code` — là khoá khớp). Ràng buộc: `Teacher` gọi kèm `replaceRecordSnapshots` → `403`; thiếu trường nào được chọn hoặc `sheetStatuses` rỗng → `400`. Bản ghi có mã không còn trong catalog mới được giữ nguyên. Response thêm `replacedRecordSnapshots` (số bản ghi thực sự đổi); audit `GoogleSheets.AssessmentsSynced` ghi kèm cờ trường và danh sách trạng thái. Lưu ý: tuỳ chọn này cố ý ghi đè tên nhóm snapshot mà `ASH-GRP-01`/`ASH-IMP-02` vốn giữ tuỳ biến.
