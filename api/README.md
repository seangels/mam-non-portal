...
- Cập nhật kết quả trực tiếp dùng `GET` và `PATCH /students/{id}/assessment-results`, chỉ dành cho `Admin`/`SuperAdmin`. `GET` đọc live `[F0.ĐG]`, ghép toàn bộ danh mục `Assessment` và trả `student: { id, studentCode, fullName, nickName, dateOfBirth, status }` cùng `items: [{ assessmentId, code, name, groupLv1Name, groupLv2Name, groupLv3Name, rowIndex, grade, note, version }]`; `version` là token opaque theo đúng hai ô nguồn của từng mục.
- `PATCH /students/{id}/assessment-results` nhận `{ items: [{ assessmentId, expectedVersion, grade, note }] }`; `grade` là `A|B|C|D|null`, note tối đa 2.000 ký tự (trim, rỗng thành null). Backend tự dò row/cột từ `ResultSource_*`, preflight toàn bộ version trước một `Values.BatchUpdate`, readback rồi rebuild latest mirror của đúng học sinh; bản ghi chỉ có note vẫn được giữ. Xung đột trả `409 AssessmentResultsVersionConflict` kèm `conflicts: [{ assessmentId, currentVersion, currentGrade, currentNote }]` và không ghi partial. Nếu Google đã ghi nhưng readback/mirror lỗi, trả `AssessmentResultsPostWriteFailed` với `googleWriteSucceeded: true`; client không được tự retry.
