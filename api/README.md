...
- Cơ chế lưu Drive (ASH-FB-W1 / G8): **luôn tạo file Google Drive mới** rồi mới **xóa file cũ theo id** trích từ link đã lưu (không dùng `Files.Update` để đè nội dung). Thứ tự này tránh mất cả hai khi lỗi giữa chừng; file cũ đã bị xóa tay trên Drive → bỏ qua lỗi `404`. Hệ quả: `PlanFileLinkPdf`/`ResultFileLinkPdf` (id + link) đổi sau **mỗi** lần upload; không cache link cũ.
