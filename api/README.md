...
- `upload-plan-pdf` là luồng PDF kế hoạch mới do UI preview HTML tạo bằng `html2pdf.js`: endpoint nhận `multipart/form-data` field `file` (`application/pdf`, tối đa 10 MB), upload PDF đó lên đúng `Student.DriveFolderId`, cập nhật `PlanFileLinkPdf` và trả lại `AssessmentSheetDetail`. Luồng này không gọi `generate-plan-pdf`, không copy/ghi `[F01]`, và nếu học sinh chưa có `driveFolderId` thì trả `409 StudentDriveFolderRequired`.
