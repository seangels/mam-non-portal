...
description: Build, verify, package, or deploy the GV Portal production release for IIS. Use only when the user explicitly invokes this skill by name; never trigger implicitly during implementation, testing, review, planning, or ordinary handoff work.
---

# GV Portal production

This is the Claude Code port of Codex's project skill at `.codex/skills/gv-portal-production/SKILL.md`. Keep the two in sync if the production/deploy process changes; the workflow and boundaries are identical, only the invoking CLI differs.
...
Check `git status`, the current commit, required tool versions, and whether the machine is the source/build machine or IIS target. Preserve unrelated changes — this workspace is also used by Codex sessions and the human user. Never read, print, store, commit, or place credentials in command history, logs, memory, artifacts, or responses.
