...
- Teacher/Admin/SuperAdmin có thể **xóa vĩnh viễn** một `AssessmentSheet` ở mọi trạng thái, kể cả `Done`. Thao tác xóa sheet và toàn bộ `AssessmentRecord` phụ thuộc chỉ trong database app, có xác nhận rõ và audit metadata an toàn; không gọi Google, không xóa file PDF/Google Drive, danh mục `Assessment`, `AssessmentSheetLatest` hoặc `AssessmentRecordLatest`. ID không tồn tại trả `404 AssessmentSheetNotFound`; thành công trả `204 No Content`.
...
- Bảng picker (`ASH-FB-W2`, 2026-09-01):
  - Trên form chỉnh sửa, picker có cột checkbox để chọn nhiều mục, checkbox `Chọn tất cả (đã chọn/tổng)` trên toolbar và hai nút `Thêm các mục đã chọn (n)`/`Bỏ các mục đã chọn (n)` (`ASH-PICKER-MULTI-01`, `ASH-PICKER-BULK-02`, 2026-09-10). `Chọn tất cả` áp dụng cho toàn bộ dòng đang khớp filter qua mọi trang, gồm cả mục đã có và chưa có; bỏ chọn chỉ tác động tập đang lọc và giữ lựa chọn ngoài filter. Lựa chọn hỗn hợp được phân loại: nút thêm chỉ xử lý mục chưa có, nút bỏ chỉ xử lý mục đã có; mỗi thao tác lưu đúng một request full-replace, thành công mới xóa selection. Các mục đã tồn tại không được thêm trùng; thao tác thêm/xóa từng dòng vẫn được giữ.
  - Toolbar picker có TagBox đa chọn riêng chỉ gồm `Trong KH` và `Chưa có trong KH`. Đây là **filter vòng 1 độc lập**, lọc local cache/view snapshot trước khi dữ liệu đi vào DataGrid; không đồng bộ với filter row/header filter vòng 2 và không hiển thị cột `Trong kế hoạch`. Chọn một trạng thái lọc nhóm tương ứng; chọn cả hai hoặc bỏ hết tương đương không lọc và TagBox giữ lại trạng thái cả hai mục được chọn. Khi records thay đổi sau add/remove, filter vòng 1 được tính lại ngay. Select-all và bulk action tính trên tập sau filter vòng 1 kết hợp filter lưới vòng 2. Nút `Đặt lại lọc lưới` chỉ reset vòng 2; reset toàn bộ mới đưa TagBox về chọn cả hai.
  - Form và picker vẫn tải toàn bộ Assessment về cache để lọc/chọn local, nhưng mỗi request nền lấy tối đa `pageSize=1000` và tiếp tục lặp đến hết `totalPages`; không dùng lô 100 dòng gây nhiều round-trip và không tải tối đa 5000 dòng trong một response.
  - Thứ tự cột mặc định trên form chỉnh sửa: `Chọn` (ghim trái) → `Thao tác` (ghim trái) → `Nội dung` → `Kết quả gần nhất` → `Nhóm 3` → `Nhóm 2` → `Nhóm tuổi` (Lv1). Ẩn mặc định `Ghi chú gần nhất`, `Mã`, `RowIndex` (vẫn bật lại được qua "Ẩn/Hiện cột"). Cho phép **kéo giãn độ rộng** và **đổi thứ tự** các cột nội dung.
...
- Khi chọn học sinh, `GET /api/v1/students/{studentId}/assessment-results` đọc toàn bộ danh mục `Assessment` và left-join `AssessmentSheetLatest`/`AssessmentRecordLatest` trong database; GET không gọi hoặc fallback Google Sheets. Dòng chưa có record latest trả grade/note `null`; `version` là token opaque sinh từ trạng thái DB của đúng dòng và client không diễn giải.
...
- `PATCH /api/v1/students/{studentId}/assessment-results` chỉ nhận `{ assessmentId, expectedVersion, grade, note }`. Backend kiểm tra toàn bộ version DB trước khi gọi Google; nếu lệch trả `409 AssessmentResultsVersionConflict`. Sau đó backend tra địa chỉ ô bằng `ResultSource_*` và so các ô Google dirty với baseline DB; nếu lệch trả `409 AssessmentResultsSourceOutOfSync` kèm grade/note ở DB và nguồn. Cả hai trường hợp đều không ghi partial.
- Các ô thực sự đổi được gửi trong một Google `Values.BatchUpdate`, đọc lại để xác nhận, rồi upsert/delete đúng các latest record liên quan. Grade rỗng nhưng có note được giữ; cả grade/note rỗng thì xóa record latest. Response canonical sau save được dựng từ DB.
- Full sync và hai luồng ghi Google `PATCH assessment-results`/`submit-results` được phép cập nhật mirror latest. `submit-results` dùng chung partial mirror updater, chỉ chạm tập Assessment vừa submit và vẫn sửa DB khi Google không có ô cần đổi; sửa working record nhưng chưa submit không làm đổi mirror.
...
- Lỗi mapping/cấu trúc/nhãn grade của nguồn trả mã lỗi nguồn cụ thể. Nếu Google đã ghi nhưng readback hoặc cập nhật mirror thất bại, response có `googleWriteSucceeded: true`; UI giữ trạng thái an toàn, cảnh báo rõ, không tự retry ghi và hướng người dùng mở đồng bộ.
- Khi còn draft, đổi học sinh/rời route/refresh phải xác nhận bỏ thay đổi. Response cũ không được ghi đè học sinh mới. Save thành công thay snapshot bằng response canonical; lỗi thường giữ draft; DB conflict có CTA tải lại, còn source drift/hậu ghi có CTA mở popup đồng bộ.
- Nút **Đồng bộ GGSheet** và cùng `GoogleSheetsSyncDialogComponent`/`GoogleSheetsService` được dùng ở cả `/assessments` và `/assessment-results`. Teacher tiếp tục đồng bộ tại `/assessments`; trang direct-results vẫn manager-only. Trên trang kết quả, sync hoạt động cả khi chưa chọn học sinh, khóa selector/edit/save khi chạy, hỏi xác nhận nếu có draft và reload snapshot DB của học sinh sau khi thành công; sync lỗi giữ draft.
