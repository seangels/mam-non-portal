...
5. **`AssessmentRecord` có hai cặp field độc lập:** `PlanGrade`/`PlanNote` (giai đoạn kế hoạch, dùng cho `[F02]`) và `FinalGrade`/`FinalNote` (kết quả cuối, dùng cho `[F03]`/`F0.ĐG`) — sửa cặp này không ảnh hưởng cặp kia; `FinalGrade`/`FinalNote` không tự động fill từ kế hoạch/latest.
