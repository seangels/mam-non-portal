...
| `ASH-FB-W2-G5` | Picker: 2 vòng lọc | Vòng 1 = panel cũ. Vòng 2 = `filter-row` + `header-filter` (dropdown cho Nhóm 1/2/3 + Kết quả). `latestGrade` dùng `calculateCellValue` (null→`Chưa có`) + `filterValues` mặc định trừ `Đạt +`. Nút "Đặt lại lọc lưới". `dxo-header-filter [allowSelectAll]` không có ở DX19 → bỏ. | **Đã code** — `ASH-FB-W2` (2026-09-01) |
