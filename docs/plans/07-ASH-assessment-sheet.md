...
3. Mở rộng `GoogleSheetsService` để hỗ trợ: **copy toàn bộ file mẫu `gen_assessment_sheet` thành file `[F01]` riêng cho mỗi `AssessmentSheet`** (lazy, tự động), ghi/cập nhật sheet `data` trong `[F01]`, điền `PlanGrade`/`PlanNote` vào sheet `khcn_template` và `FinalGrade`/`FinalNote` vào sheet `KQ_template` sẵn có trong `[F01]` để sinh PDF, ghi `FinalGrade` vào `[F0.ĐG]`, nạp lại `AssessmentSheetLatest`/`AssessmentRecordLatest` (chỉ-đọc, dùng prefill `PlanGrade`), và mở quyền đồng bộ `sync-assessments` cho `Teacher`.
...
- Frontend: trang danh sách + form tạo/sửa `AssessmentSheet` cho giáo viên, chọn học sinh, chọn plan có filter (đọc prefill `PlanGrade` từ `AssessmentSheetLatest`/`AssessmentRecordLatest`), các nút hành động (Xuất sang Google Sheet, Đồng bộ, Sinh PDF F02/F03, Cập nhật kết quả vào F0.ĐG, Đồng bộ Assessment/AssessmentSheetLatest/AssessmentRecordLatest, chuyển Open/Done), nhập `PlanGrade`/`PlanNote` (giai đoạn kế hoạch) và `FinalGrade`/`FinalNote` (giai đoạn kết quả) ở hai khu vực tách biệt.
...
- `AssessmentSheetLatest` (`AssessmentSheetLatest.cs`) và `AssessmentRecordLatest` (`AssessmentRecordLatest.cs`) — cặp entity **chỉ-đọc**, được nạp/ghi đè duy nhất bởi luồng fetch từ `[F0.data_DG]`, dùng để **prefill** `PlanGrade` khi giáo viên tạo `AssessmentSheet`/`AssessmentRecord` mới — không phục vụ mục đích nào khác, không liên quan gì tới cơ chế file `[F01]` riêng ở mục 6. `AssessmentRecordLatest.LatestGrade` vẫn là **field đơn** (không tách Plan/Final) vì đây chỉ là dữ liệu nguồn tham chiếu, không phải working record.
...
- `POST /assessment-sheets`: tạo với `studentId`, `name`, plan ban đầu (danh sách `assessmentId` đã chọn qua filter) — server snapshot `StudentSnapshot` + `AssessmentSnapshot`, và khởi tạo `PlanGrade` của từng `AssessmentRecord` bằng `LatestGrade` đọc từ `AssessmentRecordLatest` hiện có của đúng học sinh (chỉ đọc, không sửa `AssessmentRecordLatest`); `FinalGrade`/`FinalNote` để trống. Không copy file `[F01]` ở bước này (lazy, xem mục 6.1).
...
- `ASH-BE-02`: `AssessmentSheetService` — CRUD, chọn/sửa plan có filter, khởi tạo `PlanGrade` từ `AssessmentRecordLatest.LatestGrade`, chuyển `Open`/`Done`, authorization mở cho mọi vai trò.
