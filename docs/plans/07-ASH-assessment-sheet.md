...
DELETE /api/v1/assessment-sheets/{id}
...
- `DELETE /{id}` (`ASH-DEL-01`): Teacher/Admin/SuperAdmin xóa vĩnh viễn sheet ở mọi status. Service load/xóa explicit toàn bộ `AssessmentRecord` trước `AssessmentSheet` vì FK hiện hành là `Restrict`, cùng một `SaveChanges` transaction; ghi audit `AssessmentSheet.Deleted` chỉ gồm status/studentId/recordCount/cờ có PDF. Không gọi Google và không xóa Drive/PDF, `Assessment`, `AssessmentSheetLatest` hoặc `AssessmentRecordLatest`. Trả `204`; không tồn tại trả `404 AssessmentSheetNotFound`.
...
| `ASH-FB-W1-G9` | Status `Canceled` | `enum AssessmentSheetStatus` += `Canceled` (lưu string, không migration). Nhãn phân loại "không làm nữa": không transition rule (chuyển từ/đến mọi trạng thái), vẫn cho sửa như `Open` (`UpdateAsync` nhánh meta = `{Open, Canceled}`, `EnsureOpen` chặn mỗi `Done`), `DoneDate` = null, không auto-ẩn. Sync mode-2 `sheetStatuses` nhận `Canceled`. | **Đã code** — `ASH-FB-W1` (2026-08-31) |
| `ASH-FB-W1-G1` | Tạo bảng đánh giá rỗng | `POST /assessment-sheets` chấp nhận `records: []` (`CreateAsync` chỉ `EnsureDistinctIds` khi có mục). UI create bỏ picker, điều hướng vào edit sau tạo. | **Đã code** — `ASH-FB-W1` (2026-08-31) |
| `ASH-FB-W1-G8` | Cơ chế lưu Drive PDF | **Quyết định mới 2026-09-09:** có file ID cũ thì dùng `Files.Update` để replace nội dung tại chỗ, không tạo mới/xóa cũ; giữ ổn định file ID/link. Chỉ `Files.Create` khi chưa có file ID. | **Đã code** — `ASH-DRIVE-REPLACE-01`, ghi đè quyết định G8 ngày 2026-08-31 |
| `ASH-FB-W4-G7a` | Lọc danh sách theo giáo viên | `GET /assessment-sheets` + `responsibleTeacherId` (Guid). FE dropdown từ `/teachers`, Teacher không khóa. | **Đã code** — `ASH-FB-W4` (2026-09-01) |
| `ASH-FB-W4-G7b` | Multi-select danh sách | Datagrid `selection mode=multiple selectAllMode=page` — không giữ chọn qua trang; đổi filter/reset thì bỏ chọn. Paging server-side đã có sẵn. | **Đã code** — `ASH-FB-W4` (2026-09-01) |
| `ASH-FB-W4-G7c` | Bulk Action tải PDF/ảnh | `POST /assessment-sheets/pdf-archive` `{ ids, kind, format }` → file đơn (`application/pdf`/`image/png`) khi đủ điều kiện, hoặc `application/zip` khi nhiều dòng/PDF ảnh nhiều trang. **Tải + zip toàn bộ ở backend**; `format=Images` render PDF→PNG bằng `PDFtoImage` (PDFium/SkiaSharp). Tên file zip = tên gốc Drive, phẳng ở gốc. Dòng thiếu/lỗi ghi `_bo-qua.txt`. Dep mới: `PDFtoImage 5.4.0`. | **Đã cập nhật** — `ASH-PDF-DIRECT-01` (2026-09-11) |
| `ASH-FB-W2-G3` | Thứ tự nhóm Lv2 cố định | FE tự sắp: helper `assessmentGroupLv2Order`/`compareAssessmentByFixedGroupOrder` (Lv2 cố định → `rowIndex` → mã). Áp ở dropdown Nhóm 2 + default sort của picker và trang "DS Đánh giá". `ASSESSMENT_GROUP_LV2_CONFIGS` không đổi. | **Đã code** — `ASH-FB-W2` (2026-09-01) |
| `ASH-FB-W2-G4` | Picker: cột + resize + reorder | 5 cột hiện (`Thao tác→Nội dung→Kết quả gần nhất→Nhóm 3→Nhóm 2→Nhóm tuổi`), ẩn `Ghi chú gần nhất`/`Mã`/`RowIndex`. `allowColumnResizing` (`widget`) + `allowColumnReordering`; cột `Thao tác` ghim trái. | **Đã code** — `ASH-FB-W2` (2026-09-01) |
| `ASH-FB-W2-G5` | Picker: filter của lưới | Bỏ hẳn panel filter cũ (người dùng chốt); chỉ dùng `filter-row` + `header-filter` của lưới (dropdown cho Nhóm 1/2/3 + Kết quả). `latestGrade` dùng `calculateCellValue` (null→`Chưa có`) + `filterValues` mặc định trừ `Đạt +`. Toolbar lưới (qua `onToolbarPreparing` — DX19 chưa có option `toolbar`): nút "Chọn cột" mặc định + "Đặt lại lọc lưới". `dxo-header-filter [allowSelectAll]` không có ở DX19 → bỏ. | **Đã code** — `ASH-FB-W2` (2026-09-01) |
| `ASH-PICKER-MULTI-01` | Picker: chọn và thêm nhiều mục | Trên form edit, thêm cột checkbox `Chọn`; dòng đã có trong sheet bị disable. Toolbar có `Chọn tất cả (đã chọn/tổng)` ba trạng thái trên toàn bộ kết quả khớp filter qua mọi trang và nút `Thêm các mục đã chọn (n)`. Một lần bấm gửi đúng một full-replace request gồm records hiện tại + các mục mới, loại trùng theo mã; thành công mới clear selection, lỗi giữ selection để retry. Giữ thao tác thêm/xóa từng dòng. | **Đã code + verify** — 175/175 test, development build pass (2026-09-10) |
| `ASH-PICKER-BULK-02` | Records table: kết quả hiện tại/thu gọn; picker: lọc membership + bulk remove | Records table thêm `Kết quả hiện tại`, checkbox `Hiện tại` tại header `Kế hoạch` để toggle cột và checkbox `Hiện danh mục` để thu gọn/mở toàn bảng. Picker cho chọn cả dòng đã có/chưa có; TagBox toolbar `Trong KH`/`Chưa có trong KH` là filter local vòng 1 độc lập, không đồng bộ filter lưới và không hiện cột membership. Select-all và hai nút bulk add/remove áp trên tập sau filter vòng 1 + vòng 2. Mỗi thao tác gọi một full-replace request; lỗi giữ selection. | **Đã code + verify** — 185/185 test, development build hash `0b8b7ea64e37197c22ef` (2026-09-10) |
| `ASH-DEL-01` | Xóa vĩnh viễn AssessmentSheet trong app | `DELETE /assessment-sheets/{id}` xóa explicit records rồi sheet, audit metadata an toàn, không gọi/xóa Google/Drive/Assessment/latest. Form edit có nút danger với confirm phạm vi app-only và dirty warning; thành công về danh sách. | **Đã code + verify** — backend 118/118 + Release build; frontend 190/190 + development build (2026-09-10) |
| `ASH-FORM-AGE-01` | Tuổi học sinh trên form chỉnh sửa | Summary snapshot `Học sinh` nối thêm tuổi hiện tại dạng năm/tháng từ `StudentSnapshot.DateOfBirth`, tái sử dụng `calculateAgeText(..., new Date())`; không thay đổi API/schema. | **Đã code + verify** — frontend 208/208 + development build hash `65246eafe3ff45524737` (2026-09-10) |
| `ASH-CACHE-PAGE-01` | Kích thước lô cache Assessment ở form/picker | Hai loader tải toàn bộ catalog theo từng trang `pageSize=1000`, tiếp tục đến hết `totalPages`. Giữ phân trang hiển thị DataGrid và SelectBox học sinh/giáo viên như cũ; API `/assessments` đã hỗ trợ tối đa 5000 nên không đổi contract/backend. | **Đã code + verify** — focused AssessmentSheet 101/101, full frontend 208/208, development build hash `27123ece4b665d606a86` (2026-09-10) |
| `ASH-FB-W3-G6b` | Sticky bar: "Tạo mới đánh giá" | Nút ở `.form-actions` màn edit → `/assessment-sheets/new` (form trống; `PendingChangesGuard` nhắc dirty). | **Đã code** — `ASH-FB-W3` (2026-09-01) |
| `ASH-FB-W3-G6a` | Sticky bar: combo "Hoàn thành kế hoạch" | Chỉ khi bảng đã lưu `Open` + có record. `completePlan()`: lưu(Open) → `editor.status='Planed'` + lưu (save() tự `updateStatus`) → mở `plan-pdf-preview?auto=1` (preview tự `uploadPdfToDrive` rồi `goBack`). Auto, không confirm, lỗi bước nào dừng bước đó. Không backend mới. | **Đã code** — `ASH-FB-W3` (2026-09-01) |
| `ASH-FB-W5` | Màn danh sách: lọc bằng chính lưới | Bỏ panel lọc riêng. Lưới tải hết bảng đánh giá về client (`loadAllSheets`, lặp trang `pageSize` 100), `remoteOperations` off → DevExtreme tự lọc/sắp/phân trang. Bật filter row + header filter + search panel + column chooser. **Không** bật resize/reorder cột, **bỏ** `columnAutoWidth`/`columnHidingEnabled` (giảm `_synchronizeColumns`/best-fit — nguồn crash khi rời màn). Toolbar lưới (`onToolbarPreparing`): `Thêm bảng đánh giá`/`Nhập Excel`/`Bulk Action (n)`/`Đặt lại lọc lưới` + nút "Chọn cột" mặc định. Cột ngày `startDate`/`dueDate` `dataType=date`, filter row op mặc định `between`, editor `MM/yyyy` (qua `onEditorPreparing`), `calculateFilterExpression` quy khoảng về trọn tháng (`>=` đầu tháng "từ", `<` đầu tháng kế "tới"); hiển thị `M/yy`. Header filter `status` map nhãn VN. Dòng `Canceled` gạch ngang (`onRowPrepared` + class). Màu status pill phân biệt 4 trạng thái. Cột "Học sinh" thêm `studentNickName` và tuổi năm/tháng sau mã (không đậm). FE bỏ `responsibleTeacherId`/select-box GV+HS. | **Đã code** — `ASH-FB-W5` (bổ sung tuổi 2026-09-10) |
| `ASH-FB-W5` | List response += snapshot identity | `AssessmentSheetListItemResponse` thêm `StudentNickName` và `StudentDateOfBirth` từ `StudentSnapshot` — **không migration**; `search` khớp thêm nickname, UI dùng ngày sinh để tính tuổi. Là thay đổi contract nhỏ, không phá tương thích. | **Đã code** — `ASH-FB-W5` (bổ sung tuổi 2026-09-10) |
| `ASH-FB-W5` | Vá crash `_toggleBestFitMode`/`_synchronizeColumns` (deferRender sau khi hủy lưới) | **Fix gốc:** các method này ở trên **`ResizingController`** (`grid._controllers['resizing']`), không phải view `gridView`. `core/errors/dx-grid-bestfit-guard.ts` → `patchGridBestFit(grid.instance, prefix)` ghi đè own-property trên `_controllers.resizing` (bail khi detach/disposed + try/catch), gọi ở `onContentReady` của list + grid picker. **Lưới an toàn (giờ không cần):** `AppErrorHandler` (`{ provide: ErrorHandler }`) nuốt đúng lỗi + `NavigationRecoveryService` bật `/home` rồi replay route add/edit (debounce 1.5s). **Hỗ trợ:** `leaveTo()` delay điều hướng qua `rAF`+`setTimeout(0)`. Đã thử & bỏ: vá `GridView.prototype` global và vá `_views.gridView` — cả hai không ăn. `templatesRenderAsynchronously` không có trong devextreme-angular 19.2.5. | **Đã code** — `ASH-FB-W5` (2026-09-01) |
| `ASH-FB-W5` | Màu hover dòng lưới toàn theme | `ui/src/styles.scss` override hover `dxDataGrid`/`dxTreeList` từ `rgba(0,0,0,0.04)` → `rgba(51,122,183,0.18)`; không sửa file theme sinh tự động. | **Đã code** — `ASH-FB-W5` (2026-09-01) |
