...
| `ASH-FB-W4-G7c` | Bulk Action tải PDF/ảnh | `POST /assessment-sheets/pdf-archive` `{ ids, kind, format }` → `application/zip`. **Tải + zip toàn bộ ở backend** (đảo "zip ở client"); `format=Images` render PDF→PNG bằng `PDFtoImage` (PDFium/SkiaSharp). Dòng thiếu/lỗi ghi `_bo-qua.txt`. Dep mới: `PDFtoImage 5.4.0`. | **Đã code** — `ASH-FB-W4` (2026-09-01) |
