...
PATCH  /api/v1/assessment-sheets/{id}/record-group
...
- `PUT /{id}/records`: full replace danh sách `AssessmentRecord` (thêm/bớt mục, sửa `PlanGrade`/`PlanNote`/`FinalGrade`/`FinalNote`, `displayOrder`) — chặn khi `Status = Done`. Một endpoint chung cho cả 4 field; UI tách hai khu vực nhập liệu (`ASH-FE-02` cho Plan, `ASH-FE-03` cho Final) nhưng gọi cùng endpoint này. Khi thay records, backend map record cũ theo `Assessment.Code` để **giữ nguyên** `GroupLv2Name`/`GroupLv3Name` trong `AssessmentSnapshot` (tên nhóm do import khcn hoặc mục 7.1 chỉnh), không dựng lại từ `Assessment` gốc.
...
- `PATCH /{id}/record-group`: cập nhật tên group snapshot cho đúng tập `recordIds` thuộc một ô merge; có cờ `updateAssessments` để Admin/SuperAdmin tùy chọn cập nhật các Assessment gốc tương ứng trong cùng transaction. Teacher chỉ được sửa snapshot. Chỉ áp dụng khi sheet `Open`/`Planed`; `Done` bị khóa. Không ghi ngược Google Sheet trong phiên bản này.
...
| `ASH-GRP-DEC-01` | Phạm vi popup sửa group | Một popup cho đúng ô merge; mặc định sửa snapshot, tùy chọn cập nhật các Assessment xuất hiện trong ô. Không dùng dropdown và không rename toàn cục theo tên group. | Chưa code — `ASH-GRP-01` |
| `ASH-GRP-DEC-02` | Ghi ngược Google Sheet | Hiển thị checkbox nhưng disable với nhãn `Chưa hỗ trợ`; chưa implement backend write-back. Chấp nhận Assessment gốc có thể bị lần sync sau ghi đè và phải cảnh báo trên UI. | Chưa code — `ASH-GRP-01` |
| `ASH-GRP-DEC-03` | Quyền và trạng thái | Teacher/Admin/SuperAdmin sửa snapshot khi `Open`/`Planed`; chỉ Admin/SuperAdmin cập nhật Assessment gốc; `Done` khóa toàn bộ. | Chưa code — `ASH-GRP-01` |
