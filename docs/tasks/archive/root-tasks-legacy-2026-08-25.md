...
Danh mục kế hoạch: [`docs/plans/README.md`](docs/plans/README.md). Contract nền: [`docs/plans/01-BASE-admin-portal.md`](docs/plans/01-BASE-admin-portal.md).
...
- [x] `DX19-QA-02`. Chốt migration bằng automated gate, tài liệu và memory; smoke browser còn lại được người dùng bỏ qua
...
- `DX19-QA-02` browser checkpoint (2026-08-14): smoke thực tế đã phát hiện và sửa theo TDD các incompatibility DevExtreme 19 ở Angular directive scope, TextBox/DxForm value commit, Deferred confirm và nested-option lifecycle. Commit cuối `e05244b` đã qua review độc lập, focused lifecycle 5/5, full ChromeHeadlessCI 72/72 và development build pass; không thay đổi backend/REST contract, hash routing, auth flow hoặc IIS environment.
- `DX19-QA-02` runtime checkpoint (2026-08-14): login/setup/protected-route guard hiển thị đúng trên desktop/mobile; transition từ shell authenticated về `/#/login-form` không còn màn trắng hoặc lỗi `resetOption`; Attendance giữ 5 card/hàng tại 1366 px và 1 cột tại 390 px. Phiên refresh SuperAdmin đã bị thu hồi bởi lần logout trước, nên chưa thể hoàn tất successful login/logout/session restore và mutation/dirty/conflict/a11y còn lại của Admin, Teacher, Group, Student, Attendance và recovery. Người dùng sau đó tự đăng nhập SuperAdmin để tiếp tục smoke, không gửi hoặc lưu mật khẩu trong chat/repository.
- `DX19-QA-02` continuation (2026-08-14): sau khi người dùng tự đăng nhập, smoke browser xác nhận SuperAdmin auth/hash/sidebar; Admin create/password/delete và cleanup; Teacher create/detail/edit/dirty/password/delete; Group create/assign/unassign/delete; Attendance search/draft discard/card density. Không còn dữ liệu QA Admin/Teacher/Group; không tạo QA Student/Attendance/recovery.
- `DX19-QA-02` closure (2026-08-14): người dùng xác nhận bỏ qua phần smoke còn lại. Kết quả NumberBox chính sách được phân loại là automation artifact vì người dùng thao tác thực tế thấy đổi đúng. Lỗi responsive sidebar `TypeError: Cannot read properties of null (reading 'internalFields')` được chấp nhận skip cho migration này và vẫn là runtime risk đã biết; không claim console-clean/full smoke pass. Student CRUD/schedule/assignment, các case Attendance save/conflict/read-only/no-scheduled, historical recovery, responsive/a11y đầy đủ và final console gate chưa chạy. Final automated gate: Node `v14.21.3`, npm `8.19.4`, `npm --prefix ui run preinstall` pass, `npm --prefix ui run test:ci` pass 72/72, single-worker development build pass 11.77 MB hash `6138eb38f246f20a5a66`, 16 known DevExtreme/CommonJS warnings, không production/IIS/deploy.
...
- [x] `ASM-BE-01`. Mở read scope `/students` cho `Teacher` theo nhóm đang phụ trách, giữ mutation học sinh cho `Admin`/`SuperAdmin`
- [x] `ASM-FE-06`. Sửa loop reload dropdown học sinh trong form kế hoạch cá nhân do DevExtreme nhận `editorOptions` object mới mỗi change detection
- [x] `ASM-FE-07`. Đưa phần chọn mục đánh giá ra ngoài form tạo kế hoạch cá nhân, chuyển từ TagBox sang DataGrid có panel search/collapse giống màn Assessment
- [x] `ASM-FE-08`. Tách picker mục đánh giá thành component riêng, bổ sung filter nhóm tuổi/nhóm 2/nhóm 3 và nút đồng bộ GG Sheet
- [x] `ASM-FE-09`. Chuyển chọn mục đánh giá sang cột checkbox `cellTemplate`, không dùng cột select mặc định của DataGrid
- [x] `ASM-FE-10`. Highlight row mục đánh giá đã chọn khi dùng checkbox custom
- [x] `ASM-FE-11`. Bỏ nút lọc thủ công trên picker, thêm checkbox chọn tất cả mục đang hiển thị với 3 trạng thái
- [x] `ASM-FE-12`. Chuyển picker mục đánh giá sang tải toàn bộ danh sách lên client, DataGrid và filter chạy local trên cache
- [x] `ASM-FE-13`. Thêm chế độ xem tất cả hoặc chỉ những mục đánh giá đã chọn trong picker
- [x] `ASM-FE-14`. Giữ ổn định danh sách đang hiển thị ở chế độ chỉ xem mục đã chọn khi tick/bỏ tick dòng
- [x] `ASM-FE-15`. Chuyển chế độ chỉ xem mục đã chọn sang snapshot, các filter khác lọc trên snapshot này
...
- `ASM-BE-01` (2026-08-25): đã đóng contract gap cho Teacher tạo kế hoạch cá nhân bằng cách mở `GET /api/v1/students` và `GET /api/v1/students/{id}` cho role `Teacher`, nhưng chỉ thấy học sinh thuộc nhóm mà Teacher hiện phụ trách; học sinh nhóm khác hoặc chưa gán nhóm trả 404 khi get trực tiếp. `POST`/`PUT`/`PUT group`/`DELETE /students` vẫn giữ `PortalManagers` và trả 403 cho Teacher. Verification backend: `dotnet build api/AdminPortal.slnx --no-restore` pass 0 warning/error; `dotnet test api/tests/AdminPortal.UnitTests --no-restore` pass 83/83; `dotnet test api/tests/AdminPortal.IntegrationTests -c Release --no-restore` pass 26/26 sau khi Docker permission được chạy ngoài sandbox và migration upgrade test migrate tới latest model trước khi đọc `drive_folder_id`.
- `ASM-FE-06` (2026-08-25): nguyên nhân loop `/students` là `AssessmentSheetFormComponent` dùng getter trả object mới cho `[editorOptions]` của `dx-form`/`dxSelectBox`; DevExtreme 19.2.5 reconfigure editor và reload `CustomStore`. Đã đổi `studentEditorOptions`, `teacherEditorOptions`, `statusEditorOptions`, `assessmentEditorOptions`, `dateEditorOptions`, `noteEditorOptions` và `formColCountByScreen` thành object reference ổn định, đồng thời thêm regression spec giữ reference ổn định. Verification: lần đầu `npm --prefix ui run test:ci` trong sandbox bị EPERM khi npm đọc `C:\Users\sangn`; rerun ngoài sandbox pass 77/77 trước spec, development build pass hash `ef56d719257656ae84c5`, và rerun sau regression spec pass 78/78. Không chạy production build/IIS/deploy.
- `ASM-FE-07` (2026-08-25): form tạo kế hoạch cá nhân không còn chọn mục đánh giá bằng `dxTagBox` trong `dx-form`; phần chọn được đưa ra ngoài form bằng `dx-data-grid` multi-select có search panel tiếng Việt, nút lọc/đặt lại/ẩn hiện cột, paging 20/50/100 và sort server an toàn theo `code`/`name`/`rowindex`. Sticky action vẫn nằm ngoài form và hiển thị số mục đánh giá đã chọn bên trái. Đã bỏ `DxTagBoxModule` khỏi `AssessmentSheetsModule` và thêm regression spec cho đồng bộ selected row keys. Verification: `npm --prefix ui run build -- --configuration development` pass hash `cbe0604b4c38553b076f`, `npm --prefix ui run test:ci` pass 80/80; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
- `ASM-FE-08` (2026-08-25): tách phần chọn mục đánh giá thành `AssessmentPickerComponent` để form cha chỉ quản lý thông tin kế hoạch và `assessmentIds`. Picker dùng lại luồng filter như màn Assessment: text search, nhóm tuổi, nhóm 2, nhóm 3, dropdown `AssessmentGroupsService` lazy-load theo cấp cha, DataGrid multi-select và nút đồng bộ GG Sheet gọi `google-sheets/sync-assessments` rồi refresh grid. Các object/array truyền vào DevExtreme trong picker được giữ reference ổn định để tránh reload lặp. Verification: `npm --prefix ui run test:ci` pass 81/81; `npm --prefix ui run build -- --configuration development` pass hash `06eeed7c26b96079de27`; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
- `ASM-FE-09` (2026-08-25): picker mục đánh giá đã bỏ `dxo-selection`, `[selectedRowKeys]` và event selection mặc định của DataGrid. Thay vào đó cột đầu tiên dùng `cellTemplate="selectCell"` với `dx-check-box`; component tự quản lý `selectedIdSet`, emit `selectedIdsChange`, bỏ qua event programmatic khi grid render và giữ selection qua paging/filter. Đồng thời bổ sung lại nút `Lọc` trong panel filter. Verification: lần đầu build/test phát hiện TS4111 ở `SimpleChanges.selectedIds`, đã sửa thành `changes['selectedIds']`; rerun `npm --prefix ui run test:ci` pass 82/82 và `npm --prefix ui run build -- --configuration development` pass hash `6efcc0dd953921f1ee03`; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
- `ASM-FE-10` (2026-08-25): row mục đánh giá được chọn trong picker custom checkbox được highlight bằng `onRowPrepared` thêm class `assessment-picker-selected-row`, remove class khi row không còn selected để tránh reuse DOM sai màu. Khi tick/untick hoặc input `selectedIds` đổi, grid repaint để màu cập nhật ngay và vẫn giữ selection qua paging/filter. Verification: `npm --prefix ui run test:ci` pass 83/83; `npm --prefix ui run build -- --configuration development` pass hash `79ea69a4f1fc9d068557`; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
- `ASM-FE-11` (2026-08-25): panel picker đã bỏ nút `Lọc`; text search vẫn debounce và dropdown nhóm tiếp tục tự refresh theo thay đổi bộ lọc. Thêm checkbox chọn tất cả bằng native input để hỗ trợ trạng thái `checked`/`indeterminate`/unchecked trên DevExtreme 19.2.5; phạm vi là các dòng đang hiển thị trong trang hiện tại của DataGrid, còn các mục đã chọn ở trang/filter khác vẫn được giữ trong `selectedIdSet`. Verification: `npm --prefix ui run test:ci` pass 85/85; `npm --prefix ui run build -- --configuration development` pass hash `256ddb46e01ee90029d6`; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
- `ASM-FE-12` (2026-08-25): `AssessmentPickerComponent` không còn dùng `CustomStore`/remoteOperations cho DataGrid mục đánh giá. Khi mở picker, UI tải toàn bộ `/assessments` theo từng trang 100 dòng, lưu vào `allAssessments`, bind DataGrid bằng `filteredAssessments` local và derive dropdown nhóm tuổi/nhóm 2/nhóm 3 từ cache này. Đổi search hoặc dropdown chỉ lọc client-side, có hỗ trợ search không dấu qua `includesVietnamese`; không gọi lại API list. Nút đồng bộ GGSheet sau khi sync sẽ reload lại cache từ server. Verification: `npm --prefix ui run test:ci` pass 87/87; `npm --prefix ui run build -- --configuration development` pass hash `d75ecd85ead385451884`; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
- `ASM-FE-13` (2026-08-25): picker mục đánh giá có thêm dropdown `Xem tất cả` / `Chỉ những mục đã chọn` trong panel filter. Chế độ này chạy trên cache client hiện có; khi chọn `Chỉ những mục đã chọn`, DataGrid chỉ hiển thị các assessment id đang nằm trong `selectedIdSet`, vẫn kết hợp với text/group filter local và không gọi API. Hành vi khi tick/bỏ tick trong mode này được điều chỉnh tiếp ở `ASM-FE-14`. Verification: `npm --prefix ui run test:ci` pass 88/88; `npm --prefix ui run build -- --configuration development` pass hash `6d1dead66caa6a2f1d09`; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
- `ASM-FE-14` (2026-08-25): điều chỉnh hành vi `Chỉ những mục đã chọn` thành snapshot ổn định theo lần apply filter/mode gần nhất. Khi user tick/bỏ tick checkbox dòng trong mode này, UI chỉ cập nhật checkbox/highlight và `selectedIds`, không rebuild `filteredAssessments` ngay. Hành vi snapshot được làm rõ tiếp ở `ASM-FE-15`. Verification: `npm --prefix ui run test:ci` pass 88/88; `npm --prefix ui run build -- --configuration development` pass hash `888b4894c6c9dea77142`; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
- `ASM-FE-15` (2026-08-25): `Chỉ những mục đã chọn` giờ chụp một snapshot ID tại thời điểm user đổi mode sang selected. Search/nhóm tuổi/nhóm 2/nhóm 3 sau đó lọc trên snapshot này, không lọc trực tiếp theo `selectedIdSet` hiện tại; vì vậy bỏ chọn một dòng rồi tiếp tục filter vẫn không làm mất dòng khỏi tập snapshot nếu dòng đó match filter. Snapshot chỉ được chụp lại khi đổi mode; đổi checkbox chỉ repaint checkbox/highlight và emit `selectedIds`. Verification: `npm --prefix ui run test:ci` pass 88/88; `npm --prefix ui run build -- --configuration development` pass hash `bff1bb438c1bafa6024a`; chỉ còn warning DevExtreme/CommonJS đã biết. Không chạy production build/IIS/deploy.
