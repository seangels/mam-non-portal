...
| `[x]` | [`ASH-KQ-DB-SYNC-01`](details/42-ASH-KQ-DB-SYNC-01.md) | GET kết quả chuyển sang DB mirror; PATCH kiểm tra DB version + source drift trước khi ghi Google; direct save và `submit-results` dùng chung partial mirror updater. Trang kết quả dùng chung popup sync, giữ sync tại `/assessments` cho Teacher. Backend 124/124 + Release build; frontend 213/213 + development build | `ASH-KQ-DIRECT-01`, `POST /google-sheets/sync-assessments` |
...
| `[x]` | [`ASH-FB-W3`](details/33-ASH-FB-W3.md) | **Đợt 3** feedback batch (G6b+G6a) — thanh sticky màn edit (FE-only). G6b: nút "Tạo mới đánh giá" → `/assessment-sheets/new` (form trống, `PendingChangesGuard` nhắc dirty). G6a: nút combo "Hoàn thành kế hoạch" (chỉ khi bảng đã lưu `Open`) — chạy tuần tự lưu(Open) → Planed → lưu(Planed) → mở `plan-pdf-preview?auto=1` (preview tự tạo PDF + `upload-plan-pdf` lên Drive rồi `goBack`). Không confirm; lỗi bước nào dừng bước đó. FE `test:ci` 156/156 + dev build `601fe22f`. Không backend. req09 §7 + plan07 §13 + memory synced; chưa commit | `ASH-FB-W2` |
| `[x]` | [`ASH-FB-W2`](details/32-ASH-FB-W2.md) | **Đợt 2** feedback batch (G3+G4+G5) — đại tu picker + thứ tự nhóm Lv2 (FE-only). G3: helper `assessmentGroupLv2Order`/`compareAssessmentByFixedGroupOrder` (Lv2 cố định → `rowIndex` → mã); áp ở dropdown Nhóm 2 + default sort của picker và trang "DS Đánh giá". G4: picker 6 cột (ẩn `code`/`rowIndex`), `columnResizingMode="widget"`. G4 thêm reorder cột (`allowColumnReordering`), thứ tự `Thao tác→Nội dung→Kết quả→Nhóm3→Nhóm2→Nhóm1`, ẩn `Ghi chú`/`Mã`/`RowIndex`. G5: bật `filter-row` + `header-filter` + nút "Đặt lại lọc lưới"; cột `Kết quả gần nhất` dùng `calculateCellValue` (null→`Chưa có`) + `filterValues` mặc định trừ `Đạt +`; `dxo-header-filter` DX19 không có `allowSelectAll` nên bỏ. FE `test:ci` 154/154 + dev build `9096fa55`. Không backend. req09 §5/§16 + plan07 §13 + memory synced; chưa commit | `ASH-FB-W4` |
| `[x]` | [`ASH-FB-W5`](details/34-ASH-FB-W5.md) | **Tinh chỉnh màn danh sách** (nối sau Đợt 4) — bỏ panel lọc riêng, lọc bằng chính lưới: lưới tải hết sheet về client, bật `filter-row`+`header-filter`+`search-panel`+`column-chooser` (không resize/reorder), toolbar lưới chứa `Thêm`/`Nhập Excel`/`Bulk Action (n)`/`Đặt lại lọc lưới`. Cột ngày `startDate`/`dueDate` lọc `between` dạng chọn tháng `MM/yyyy` (khoảng bao trọn tháng), lưới hiển thị `M/yy`. Dòng `Canceled` gạch ngang; status pill 4 màu. Cột "Học sinh" thêm `studentNickName` sau mã (BE `AssessmentSheetListItemResponse` += `StudentNickName`, không migration; `search` khớp nickname). **Vá crash** `_toggleBestFitMode`/`_synchronizeColumns` (deferRender sau khi hủy lưới — list **hoặc** grid picker trong form): fix gốc = `patchGridBestFit()` (`core/errors/dx-grid-bestfit-guard.ts`) ghi đè own-property trên **`grid._controllers['resizing']`** (không phải `_views.gridView` — bọc nhầm object nên bản trước không ăn), gọi ở `onContentReady` list + picker. Lưới an toàn: `AppErrorHandler` (`{provide: ErrorHandler}`) nuốt đúng lỗi + `NavigationRecoveryService` bật `/home` rồi replay route add/edit; `leaveTo()` delay điều hướng (rAF). Log người dùng xác nhận hết `NavigationError`. Màu hover dòng lưới toàn theme → xanh dương `rgba(51,122,183,0.18)` (`styles.scss`). Form `dueDate` chọn tháng → snap value về **ngày cuối tháng** (`startDate` giữ đầu tháng). BE build Release 0/0 + unit 104/104; FE `test:ci` **158/158** + dev build `edbaaf49`. Integration not run (Docker off). req09 §16.1/§16.2 + plan07 §13 + README + FE/BE memory synced. Commit `7d6b44f` (gồm cả `[colSpan]="2"` người dùng yêu cầu) + follow-up. | `ASH-FB-W4` |
...
## Drive upload delta — owner: `backend`

| Status | Mã | Việc cần làm | Phụ thuộc |
|---|---|---|---|
| `[x]` | [`ASH-DRIVE-REPLACE-01`](details/35-ASH-DRIVE-REPLACE-01.md) | Đảo quyết định G8: có file PDF Drive cũ thì replace nội dung tại chỗ bằng `Files.Update`, giữ nguyên ID/link; chỉ tạo mới khi chưa có file ID, không xóa file cũ. Backend unit 117/117 + Release build sạch; integration không chạy (Docker off), chưa Google live smoke | `ASH-FB-W1`, `upload-plan-pdf`, `upload-result-pdf` |
