...
4. ✅ **Cột ngày `Bắt đầu`/`Hạn hoàn thành`:** `dataType="date"`, ô lọc filter row chọn theo tháng — `onEditorPreparing` ép `displayFormat="M/yyyy"` + `calendarOptions.maxZoomLevel="year"`. `[calculateFilterExpression]="monthGranularFilterExpression"` xử lý **mọi** toán tử ở mức tháng: `between [từ,tới]` → `>=` đầu tháng "từ" và `<` đầu tháng kế "tới"; `>=` → `>=` đầu tháng; `<=` → `<` đầu tháng kế (hết tháng); `=` → `[>=` đầu, `<` đầu kế `]`; `>`/`<` tương tự. Mặc định: `startDate` `selectedFilterOperation="<="`, `dueDate` `">="`. `filterOperations` mỗi cột để toán tử mặc định lên đầu. Lưới hiển thị `M/yyyy` (helper `monthText`, tháng không đệm `0`). Header filter tắt cho 4 cột ngày.
   - Form nhập (`assessment-sheets-form`): cả `startDate`/`dueDate` `displayFormat="M/yyyy"`. `dueDate` dùng `dueDateEditorOptions` (kế thừa `dateEditorOptions` + `onValueChanged`) — người dùng chọn tháng nào thì snap value về **ngày cuối tháng** đó (`new Date(y, m+1, 0)`), chỉ khi `event.event` (user thao tác) để không đụng lúc load. `startDate` giữ ngày đầu tháng.
...
- ✅ `dotnet test api/tests/AdminPortal.UnitTests -c Release --no-build` → **104/104**.
- ✅ `docs/requirements/09` §16.1 + §16.2, `docs/plans/07-ASH` §13, `api/README.md`, `.agents/{frontend,backend,shared}/MEMORY.md` cập nhật.
- ✅ Smoke crash: log người dùng xác nhận sau fix — `bestFitGuard — bỏ qua _synchronizeColumns()` rồi `NavigationEnd` thẳng, không còn `NavigationError`/`AppErrorHandler`/`/home`.
- ⬜ Smoke thủ công còn lại (filter row/header/ô tìm kiếm/column chooser; toolbar 4 nút; lọc `Bắt đầu`/`Hạn hoàn thành` từ tháng → tới tháng; dòng `Canceled` gạch ngang; cột Học sinh có nickname; hover dòng lưới xanh dương): chưa chạy.
- ✅ Commit `7d6b44f` (gồm cả `[colSpan]="2"` người dùng yêu cầu gộp).
