...
- ✅ Gom điều kiện khóa vào helper dễ đọc: `canMutateAssessmentSheetRecords`, `canEditAssessmentSheetRecordValues`, `canMutateRecords`, `recordValueControlsDisabled`.
- ✅ Đã rà lại các chỗ add/remove/edit record: add/remove khóa cả `Planed` và `Done`; edit `FinalGrade`/`FinalNote` chỉ khóa khi `Done`.
...
- ✅ Chạy `npm --prefix ui run test:ci` — pass 109/109.
- ✅ Chạy `npm --prefix ui run build -- --configuration development` — pass, hash `2cdd3e9076b4c5566eb0`.
