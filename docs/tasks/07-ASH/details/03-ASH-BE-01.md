...
- Áp dụng khoá upsert đã chốt ở `ASH-DEC-05`: unique index trên `AssessmentSheetLatest.StudentId`; unique index trên `AssessmentRecordLatest` theo (`AssessmentSheetLatestId`, mã mục đánh giá). **Kết quả thực tế:** EF Core từ chối index trực tiếp trên `x.AssessmentSnapshot.Code` (sub-property của complex type map JSON) — báo lỗi ngay khi `dotnet-ef migrations add` chạy. Đã thêm field scalar mới `AssessmentRecordLatest.AssessmentCode` (`string`, required, `maxLength 50`) làm cột index thay thế; `AssessmentSnapshot` vẫn giữ nguyên dạng jsonb cho mục đích hiển thị/lưu trữ đầy đủ.
