...
- 2026-08-25: Assessment picker now has a local view-mode dropdown with `Xem tất cả` and `Chỉ những mục đã chọn`. The mode is part of the client-side filter pipeline over `allAssessments`/`filteredAssessments`; selected-only mode combines with text/group filters, performs no API call, and refreshes immediately when `selectedIds` changes or a custom checkbox is unticked. No backend/REST contract changed. Verification: `npm --prefix ui run test:ci` pass 88/88 and `npm --prefix ui run build -- --configuration development` pass hash `6d1dead66caa6a2f1d09`. Only known DevExtreme/CommonJS warnings; no production/IIS/deploy.
