...
- 2026-08-25: Assessment picker selected-only mode is now a stable displayed snapshot after the most recent explicit filter/mode/cache refresh. Changing row checkboxes updates `selectedIds`, checkbox state, and row highlight, but no longer reapplies `filteredAssessments` immediately; unchecked rows can remain visible until the user changes filter/view mode or cache reloads. No backend/REST contract changed. Verification: `npm --prefix ui run test:ci` pass 88/88 and `npm --prefix ui run build -- --configuration development` pass hash `888b4894c6c9dea77142`. Only known DevExtreme/CommonJS warnings; no production/IIS/deploy.
