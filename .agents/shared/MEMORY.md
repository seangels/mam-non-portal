...
- Attendance compact-card UI epic draft `AUI` is under review at `plans/05-AUI-attendance-compact-cards.md`; no source implementation has started. It proposes a dense responsive card grid without API changes. The `Chưa điểm danh` behavior is explicitly pending because it is not an API status and can change the Missing first-save workflow; `AUI-DEC-01/02` must be confirmed first.
