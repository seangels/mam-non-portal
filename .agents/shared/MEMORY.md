...
- Attendance compact-card epic draft `AUI` is under review at `plans/05-AUI-attendance-compact-cards.md`; no source implementation has started. `AUI-DEC-01/02/03` are locked: add persisted `AttendanceStatus.Unmarked`, keep Missing defaults Present/OneToOneHour by schedule and let users explicitly choose Unmarked; new AbsentHalfDay writes no longer use `halfDayPart`, still require excused/unexcused, with details in notes. Implementation now spans API/DB/UI and must preserve legacy half-day data.
