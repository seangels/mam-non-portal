...
- Attendance compact-card epic `AUI` is fully planned and ready to implement at `plans/05-AUI-attendance-compact-cards.md`; no source implementation has started. `AUI-DEC-01`–`08` are locked: persisted `AttendanceStatus.Unmarked`; Missing defaults Present/OneToOneHour by schedule; new AbsentHalfDay writes omit `halfDayPart` but retain excused/unexcused; cards identify only `nickname · studentCode`; recovery is outside the v1 redesign; UI notes max is 200 while the API remains 2,000 and unchanged longer legacy values must not be truncated; the desktop grid is fluid with a five-card target at 1366 px; status colors follow the reference groups through accessible design tokens. Implementation spans API/DB/UI and preserves legacy half-day data.
