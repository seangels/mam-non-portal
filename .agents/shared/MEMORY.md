...
- Teacher management epic `TCH` is in planning at `plans/03-TCH-teacher-management.md`; no product implementation has started. The draft keeps account fields canonical in `User`, professional fields in `Teacher`, group assignment exclusively in `student-groups`, and attendance snapshots immutable. `TCH-DEC-01` through `TCH-DEC-12` are proposals awaiting user review before `TCH-00`; do not treat the draft DTOs/endpoints as locked contract yet.
